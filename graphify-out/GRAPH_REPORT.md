...
## Corpus Check
- 289 files · ~75,979 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1107 nodes · 1096 edges · 246 communities detected
...
- `FolderSerializer` --uses--> `UserSerializer`  [INFERRED]
  backend/serializers/folder.py → backend/serializers/user.py
- `Meta` --uses--> `UserSerializer`  [INFERRED]
  backend/serializers/folder.py → backend/serializers/user.py
- `Folder` --uses--> `User`  [INFERRED]
  backend/models/folder.py → backend/models/user.py
- `DeckViewSet` --uses--> `IsOwnerPermission`  [INFERRED]
  backend/views/deck.py → backend/permissions.py
- `FolderViewSet` --uses--> `IsOwnerPermission`  [INFERRED]
  backend/views/folder.py → backend/permissions.py
...
## Knowledge Gaps
- **141 isolated node(s):** `Run administrative tasks.`, `SKIP_REDIS`, `SMTP backend that uses certifi's CA bundle.      Needed on macOS/environments wh`, `Attempts to find and return a user using the given validated token.`, `Django settings for core project.  Generated by 'django-admin startproject' usin` (+136 more)
...
- **Thin community `Community 229`** (1 nodes): `mod.rs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 230`** (1 nodes): `mod.rs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 231`** (1 nodes): `mod.rs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 232`** (1 nodes): `mod.rs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 233`** (1 nodes): `mod.rs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 234`** (1 nodes): `mod.rs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 235`** (1 nodes): `mod.rs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 236`** (1 nodes): `mod.rs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `DeckViewSet` connect `Community 1` to `Community 6`?**
...
- **Are the 5 inferred relationships involving `Deck` (e.g. with `User` and `Term`) actually correct?**
