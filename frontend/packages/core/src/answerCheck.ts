...
export type AnswerStatus = "correct" | "accepted" | "incorrect";
 ...
 */
export function normalizeAnswer(str: unknown): string {
  if (str == null) return "";
 ...
 */
function expandOptional(raw: string): string[] {
 ...
 */
export function buildVariants(correctAnswer: unknown): string[] {
...
export function diffAnswer(
  ...
  const user: DiffSegment[] = [];
  const correct: DiffSegment[] = [];
  const pushSeg = (arr: DiffSegment[], ch: string, match: boolean) => {
