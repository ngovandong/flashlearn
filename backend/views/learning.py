...
from ..services import LearningService, TermService
...
class LearningViewSet(FlexibleViewSet):
    ...
    def perform_create(self, serializer):
        ...
        LearningService.create_or_touch_progress(self.request.user, term_id, user_id)
    ...
    def correct(self, request, *args, **kwargs):
        LearningService.record_correct(self.get_object(), request.user)
        return Response(status=status.HTTP_200_OK)

    @action(detail=True, methods=["PUT"])
    def incorrect(self, request, *args, **kwargs):
        LearningService.record_incorrect(self.get_object(), request.user)
        return Response(status=status.HTTP_200_OK)

    @action(detail=True, methods=["PUT"])
    def remember(self, request, *args, **kwargs):
        LearningService.toggle_remember(self.get_object())
        return Response(status=status.HTTP_200_OK)

    @action(detail=True, methods=["PUT"])
    def priority(self, request, *args, **kwargs):
        adjust_point = int(request.data.get("adjust_point", 0))
        LearningService.adjust_priority(self.get_object(), adjust_point)
        return Response(status=status.HTTP_200_OK)
    ...
    def get_latest_learned_term(self, request, *args, **kwargs):
        ...
        return Response(LearningService.get_latest_learned_term_info(request.user, deck_id, page_size))
