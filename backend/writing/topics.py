"""Curated writing prompts loaded from ``topics.json``.

Mirrors :mod:`backend.speaking.topics`: prompts are stored in a flat JSON file
(no database table) and served by the topic suggester, filtered by the learner's
CEFR level. The file is read once and cached in memory.
"""
...
def random_for_level(level: str | None = None, exclude=None, limit: int = 5) -> list[str]:
    """Random prompt texts for ``level``, backfilling from other levels if sparse."""
